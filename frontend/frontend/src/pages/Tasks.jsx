...
const Tasks = () => {
  ...
  // Fix here: use selectedTask._id instead of undefined task._id
  const handleCommentSubmit = async (commentText) => {
    ...
    try {
      ...
      // Optionally update tasks or selectedTask state here if you want to reflect new comments immediately
          ...
          <table className="table w-full">
            ...
            <tbody>
              {tasks.length > 0 ? (
                tasks.map((task, index) => {
                  ...
                  return (
                    <tr key={task._id} className={isCompleted ? 'opacity-60' : ''}>
                      ...
                      <td className={isCompleted ? 'line-through' : ''}>{task.project?.name || 'N/A'}</td>
                ...
                <tr>
                  <td colSpan="7" className="text-center p-4">No tasks assigned yet.</td>
                </tr>
              )}
            </tbody>
          </table>
